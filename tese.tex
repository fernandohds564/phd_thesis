...
\chapter{Impedance Budget}\label{cap:impedance_budget}
    ...
    While the single--bunch tune shifts are mostly defined by the broad--band impedance of the whole spectrum, their multi--bunch counterpart depends majorly on the low--frequency components of the dipole impedance and on the zero--frequency component of the quadrupolar impedance, because, as shown in the small graphics of Figure~\ref{fig:dipole_hor_imp_budget} and Figure~\ref{fig:dipole_vert_imp_budget}, the impedance in this region is two or three orders of magnitude larger than the high frequency part of the spectrum, being dominated by the resistive--wall contributions from the standard chamber and the undulators.
    ...
    \end{figure}
    shows the tune shift divided by the bunch current in uniform filling as function of the number of bunches filled with charge. Note that if the impedance were broad banded the curve in the graphic should be a horizontal line. At single--bunch operation mode the incoherent tune--shifts are completely negligible compared to the incoherent one, because of the much smaller quadrupolar impedance of the machine
...
\section{Longitudinal Plane}\label{sec:longitudinal_plane}
    ...
    In this section the studies of the longitudinal dynamics of the Sirius storage ring will be discussed
...
\subsection{Impedance Model}\label{ssec:imp_model}
