...
\section{Coherent Motion (Instabilities)}
    ...
    Assuming the ring is uniformly filled with $M$ equally spaced bunches, by symmetry, the equilibrium distribution must be the same for all of them and the time evolution of the perturbations in each bunch must be equal, only a phase difference between them is allowed. Since the system is periodic, we can expand this phase difference in the coupled--bunch base defined by
    \begin{align}
        \mu \frac{n-l}{M}&& \text{with}&& \mu \in [0, M-1]
    \end{align}
