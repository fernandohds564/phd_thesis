...
\chapter{Impedance Budget}\label{cap:impedance_budget}
    ...
    \begin{description}
        ...
        \item[Valve Block:] All the valves of the storage ring will have a Blw BPM Blw block followed by a Radiation Mask in the upstream direction and a bellow downstream. For the same reasons as the previous component, all these elements were simulated as single block.~\citeonline{Duarte2017a} shows a Figure of these components and the impedance comparison of the sum of individual impedances and the block impedance, demonstrating the differences among both approaches.
    ...
    \begin{figure}
        ...
        \label{fig:ph1_kicky}
    \end{figure}
    shows the $\beta_{x,y}$ weighted single bunch kick factor for the horizontal and vertical dipole and quadrupole impedances. Note that the impedance in both planes are very similar to each other, due to the predominating round chamber of the machine. With exception of the \gls{ids} the main differences come from the local betatron functions at the position of the impedance.
