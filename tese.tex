...
\section{Longitudinal Plane}

\section{Transverse Plane}

    Differently from the longitudinal plane, in the transverse plane there will be coupled--bunch instabilities because of the long--range wake field of the resistive--wall impedance from the standard beam pipe and from the \glspl{id}. On the other hand, the additional energy loss by radiation introduced by the \glspl{id} also increase the damping rate in all planes, and change the equilibrium parameters of the machine. These calculations were made for the Sirius storage ring assuming all \glspl{id} were in their configuration of maximum vertical field and the main results can be found in~\cite{Sirius2013}. When these effects are included in the instability threshold calculations, the increased damping rates help pushing the threshold to higher currents, but, since the \glspl{id} will be constantly moving, changing the configuration of the fields, in practice the damping rates of the machine will oscillate between the bare machine values and the maximum value presented in the mentioned reference. For this reason we decided to adopt the conservative approach and do not include the effect of the \glspl{id} on the damping time to calculate the instability thresholds. Figure~\ref{fig:ph1_coup_bunch}
    \begin{figure}
        \centering
    ...
    \begin{table}
        ...
        \end{tabular}
    \end{table}
    shows the current thresholds at zero chromaticity for the coupled--bunch instabilities for different assumptions on the effect of the \glspl{id} to show their influence on the impedance budget and also the contribution of the additional damping on the stabilization of the beam. The vertical threshold is almost twice as large as the one for the horizontal plane, even for the bare machine impedance. This is a consequence of the larger average vertical betatron function of the machine. Table~\ref{tab:ph1_coup_bunch} shows another feature, already discussed in chapter~\ref{cap:impedance_budget}, which is the relatively weak effect of the \glspl{id} on the instability threshold, given their gap is of the order of five times smaller than the standard vacuum chamber and the resistive wall goes with $1/b^3$, which, again, shows the advantage of having very small  betatron functions at the straight sections.
    ...
    \end{figure}
    shows the effect of the chromaticity on the most unstable mode of the coupled--bunch instability calculate with two different approaches. One using the approximate expression of equation~\ref{eq:trans_coupled_bunch_instability} for azimuthal modes 0 and 1, which are the diagonal terms of the mode--coupling matrix, given by equation~\ref{eq:trans_mode_coupling_instability}, and another by solving the mode--coupling matrix, considering the first 7 radial modes and azimuthal modes from -7 to 7. There is a very good agreement between the two approaches at low chromaticities, when the azimuthal mode 0 dominates the instability, but at larger values of chromaticities the simplified approach overestimates the thresholds. This study shows that the beam will be stable when the ring is operated with chromaticities above 2.3 and 1.4 in the vertical and horizontal planes, respectively. Such dependency was foresaw a few years ago, which allowed the optimization of the non--linear optics of the storage ring to operate at this chromaticity, which avoids the usual problems of lifetime and dynamic apperture reduction related to this type of operation.

    Even though the study above shows the possibility of operating the ring in stable conditions with a increased chromaticity, the most reliable way to solve this problem and which has demonstrated to be successful in dozens of \gls{3gls} along the last two decades is the use of transverse bunch--by--bunch feedback systems. Sirius will have two of these systems, one for the horizontal plane and another for the vertical. The kicker used for them will be striplines with shunt impedance of \SI{70}{\kilo\hom\per\meter} and the amplifier will have maximum power of \SI{75}{\watt} of which approximately \SI{30}{\watt} will be aplicable to the beam, due to cable losses.
