...
\chapter*[Conclusions]{Conclusions}
    ...
    In this work we modeled the impedance of some of the main components of the Sirius storage ring with simplified models and gathered the results of other components whose calculations were done in detail with 3D solvers of Maxwell equations, to form the initial impedance budget of the machine. This data was used to calculate the current thresholds for single and multi-bunch instabilities for the phase one of operation.

    One of the most important components of the impedance budget is the standard chamber of the storage ring, which has a small transverse cross-section size, only \SI{12}{\milli\meter} radius, and will be \gls{neg}--coated along all its extension. In this work we used semi--analytic formulas from the literature to compute the impedance of this component, which allowed us to evaluate the effect of \gls{neg} in the chamber under various possible scenarios related to its thickness and conductivity. It was verified that variations in the conductivity close to the nominal value of \SI{1}{\mega\siemens}, do not have a large impact on the current threshold of the \gls{tmci} instability. On the other hand, thickness variations
