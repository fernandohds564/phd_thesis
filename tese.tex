...
\chapter{Impedance Modeling}\label{cap:impedance_modeling}
    ...
    In this chapter the modeling of the impedance of some components of the storage ring will be described. The components described below are the ones for which the modeling is not trivial and required detailed analysis of some key aspects. The gathering of all the components and analysis of the whole impedance budget will be performed in the next chapter.
...
\subsection{Standard Impedance Budget Effects}\label{ssec:imp_bud_effects}
    ...
    Figure~\ref{ph1_microwave_tracking} also reveals a small sudden change in the average longitudinal position of the beam as function of the current from the simulations with \SI{2.2}{\milli\ampere} and \SI{2.4}{\milli\ampere} in both simulations presented there. This could be an indication of the weak instability predicted by the frequency domain analysis, but no other result besides this one corroborates with such interpretation, and further studies are required to confirm or reject this hypotesis.
...
\subsection{Inclusion of \gls{csr} in the Budget}
    ...
    Before including the \gls{csr} in the Sirius impedance budget, we performed simulations using only this source of impedance to test if we could find the same threshold as predicted by the theory.
