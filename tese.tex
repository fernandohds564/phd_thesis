...
\section{Transverse Plane}
    ...
    Figure~\ref{fig:ph1_chrom_tmci} shows that the chromaticity also have a strong effect on the single bunch instabilities, where positive values reduces the current threshold in both planes, but also the growth rates. It is expected that this reduced growth rate will improve the effectiveness of the control of this instability by the bunch--by--bunch feedback system, as shown experimentaly at ALS~\cite{Byrd1997} and at Diamond~\cite{Koukovini-Platia2017}. It is known from the theory, for example as explained by~\citeonline{Cullinan2016} or by~\citeonline{Lindberg2016}, that the longitudinal impedance has an effect on the transverse single--bunch stability, due to the reduction of the average synchrotron tune, which tends to decrease the threshold and by the creation of the 
